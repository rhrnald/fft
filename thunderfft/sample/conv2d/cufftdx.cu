...
static constexpr int loop_x = 2;
static constexpr int loop_y = 2;
                                   ...
                                   SMLayout smem_layout, ICTATiler input_cta_tiler, OCTATiler output_cta_tiler)
{
    ...
    extern __shared__ complex_type tile[];
    ...
    auto reg = make_tensor<cufftdx::complex<real_type>>(make_shape(Int<tile_size>{}), LayoutRight());
    ...
    for (int loop_y_idx = 0; loop_y_idx < loop_y; ++loop_y_idx) {
        for (int loop_x_idx = 0; loop_x_idx < loop_x; ++loop_x_idx) {
            ...
            for (row_index = 0; row_index < tile_size; ++row_index) {
                reg(row_index) = input_tile_gmem_tensor(row_index, col_index);
            }
            ...
            for (col_index = 0; col_index < tile_size; ++col_index) {
                reg(col_index) *= filter_gmem_tensor(row_index, col_index); // filter transposed
                // reg(col_index) *= complex_type(1.3f, 2.4f);
            }
