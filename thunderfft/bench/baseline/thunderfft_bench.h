...
__global__ void ThunderFFT_kernel_ir<float,256,16>(
    ...
    // gmem -> smem (input)
    for(int i=warpid; i<batch_per_block; i+=blockDim.y) {
        for(int j=threadid; j<N; j+= blockDim.x) {
            // s_in[j] = d_input[blockid * N * batch_per_block + j];
            s_in[(i*4 + j/64) * (64+4) + j % 64] = d_input[blockid * N * batch_per_block + N * i + reverse_bit_groups<2,LOG2P_builtin<N>>(j)];
        }
    }
    ...
    for (int r = 0; r < inside_repeats; ++r) {
        ...
        thunderfft::detail::unit::fft_kernel_r16_b16<true>(reg, dW);
...
template <>
__global__ void ThunderFFT_kernel_ir<float,1024,1>(
    vec2_t<float>*       d_input,
    vec2_t<float>*       d_output,
    const float*         __restrict__ dW,
    unsigned                  inside_repeats)  {
    ...
    for (unsigned i = 0; i < batch_per_block; i++) {
        for(unsigned j=threadIdx.x; j<N; j+= blockDim.x) {
            s_in[ j ] = d_input[blockid * N * batch_per_block + reverse_bit_groups<2,6>(j)];
            // s_in[i * (N+pad)+ j] = d_input[b * N * batch_per_block + i * N + j];
        }
    }
    ...
    for (unsigned r = 0; r < inside_repeats; ++r) {
        ...
        auto *i_0 = (s_in+(laneid/4)*(64+4) );
        auto *i_1 = (s_in+(laneid/4+8)*(64+4));
    ...
    // for (unsigned r = 0; r < inside_repeats; ++r) {
        ...
        auto *o_0 = (s_out+(laneid/4)*(64+4));
        auto *o_1 = (s_out+(laneid/4+8)*(64+4));
        ...
        for(int i=0; i<ept/2; i++) {
            int col = i + (laneid%4)*16;
            int rev_row1 = laneid/16 * 4 + ((laneid/4) % 4) * 16;
            int rev_row2 = rev_row1+8;
            reg2[i] = cmul(reg2[i], W(col * rev_row1, 4096));
            reg2[i+ept/2] = cmul(reg2[i+ept/2], W(col * rev_row2, 4096));
        }
    ...
    // }
        ...
        auto * o_01 = (s_out+(laneid/4)*(64+4));
        auto *o_11 = (s_out+(laneid/4+8)*(64+4));
        thunderfft::detail::unit::reg2smem(reg2, o_01, o_11, 1);
        __syncthreads();
    }
