...
ThunderFFT_kernel_reg<float, 1024, 1, true>(vec2_t<float>* __restrict__ reg, vec2_t<float>* W_precompute, void *workspace) {
    ...
    // using L_out = layout_t<64, 16, 16, 1, 16, 1, true>;
    // ThunderFFT_smem2reg_N64<float, L_out>(reg, smem);
    ...
    //     row = reverse_bit_groups<2,4>(row);

    //     int col0 = laneid/4 + (i/4) * 16;
    //     int col1 = col0 + 8;
    //     reg[i] = cmul(reg[i], W(col0 * row, 1024));
    //     reg[i+ept/2] = cmul(reg[i+ept/2], W(col1 * row, 1024));
    // }
...
template<>
__device__ __forceinline__ void
ThunderFFT_kernel_reg<float, 1024, 1, false>(vec2_t<float>* __restrict__ reg, vec2_t<float>* W, void *workspace) {
    
}

template<>
__device__ __forceinline__ void
ThunderFFT_kernel_reg<half, 1024, 1, true>(vec2_t<half>* __restrict__ reg, vec2_t<half>* W, void *workspace) {
    
}

template<>
__device__ __forceinline__ void
ThunderFFT_kernel_reg<half, 1024, 1, false>(vec2_t<half>* __restrict__ reg, vec2_t<half>* W, void *workspace) {
