...
__device__ void fill_reg_b_half(half2 b[], int stride_log2, int stride,
    ...
    b[0] = make_half2(W_ptr[(index1 & (4 * stride - 1)) * (16 / stride)].x,
                      W_ptr[(index2 & (4 * stride - 1)) * (16 / stride)].x);
}
...
template <unsigned int N>
__device__ void fill_reg_b_branch(float b[], int stride_log2, int stride,
                                  int i_perm, int j_perm, int k,
                                  const cuFloatComplex *__restrict__ W_ptr) {
    int i = (threadIdx.x / 8 - i_perm) & 3;
    int j = (threadIdx.x % 4 - j_perm) & 3;
    ...
    int index2 = (j1 / 2) * (k + stride * (i1 / 2)) + stride * (i1 & 1) -
                 stride * (j1 & 1);
    auto w =
        W(j * (k + stride * i) + stride * ((threadIdx.x / 4) & 1), 4 * stride);
...
template <typename T>
__device__ __forceinline__ typename std::enable_if<sizeof(T) == 4, void>::type
permute_predicated(T &a, T &b, T &c, T &d, int pattern) {
    ...
    asm volatile("{\n\t"
                 ...
                 // b <-> d
                 "@p_bd { .reg .b32 t; mov.b32 t, %1; mov.b32 %1, %3; mov.b32 "
                 "%3, t; }\n\t"
                 // a <-> c
                 "@p_ac { .reg .b32 t; mov.b32 t, %0; mov.b32 %0, %2; mov.b32 "
                 "%2, t; }\n\t"
                 // a <-> b,  c <-> d
                 "@p_abcd {\n\t"
                 "  .reg .b32 t;\n\t"
...
__device__ void permute_radix4_shift_half(half2 &a, half2 &b, half2 &c,
                                          half2 &d, int pattern) {
    ...
    reinterpret_cast<uint64_t *>(tmp_x)[0] =
        ror64(reinterpret_cast<uint64_t *>(tmp_x)[0], s);
    reinterpret_cast<uint64_t *>(tmp_y)[0] =
        ror64(reinterpret_cast<uint64_t *>(tmp_y)[0], s);
...
template <int N>
__device__ void fft_kernel_r64_b16(float *reg, const cuFloatComplex *W_4096) {
...
#pragma unroll
        for (int j = 0; j < N_DEVICE_CONST / RADIX_DEVICE_CONST; j++) {
            ...
            reg_frag_a[1] =
                reg[j * 2 + 2 * N_DEVICE_CONST / RADIX_DEVICE_CONST];
            reg_frag_a[2] = reg[j * 2 + 1];
            reg_frag_a[3] =
                reg[j * 2 + 1 + 2 * N_DEVICE_CONST / RADIX_DEVICE_CONST];
            // w = w_4stride
            // b = [ w^ i ( k + Nj) ] ^ T
            int j_perm;
            if (stride >= 4)
            ...
            int k = j % stride;
            fill_reg_b<N>(reg_frag_b, i * 2, stride, i_perm, j_perm, k, W_4096);
        ...
        if (i < ITER_DEVICE_CONST - 1) {
            for (int jk = 0; jk < 8; jk++) {
                ...
                permute_radix4_tmp(
                    ...
                    reg[2 * (k + j + stride * 2)],
                    reg[2 * (k + j + stride * 2) + 1],
                    reg[2 * (k + j + stride * 3)],
                    reg[2 * (k + j + stride * 3) + 1], laneid & 3);
            }
        }
        // if (i < ITER_DEVICE_CONST - 1) {
        //     for (int jk = 0; jk < 8; jk++) {
        ...
        //                              reg[2*(k + j + stride)], reg[2*(k + j +
        //                              stride)+1], reg[2*(k + j + stride * 2)],
        //                              reg[2*(k + j + stride*2)+1], reg[2*(k +
        //                              j + stride * 3)],reg[2*(k + j +
        //                              stride*3)+1],
        //                                 laneid & 3);
        //     }
        // }
...
#pragma unroll
        for (int j = 0; j < N_DEVICE_CONST / RADIX_DEVICE_CONST; j++) {
            ...
            fill_reg_b_half<N>(reg_frag_b, i * 2, stride, i_perm, j_perm, k,
                               W_ptr);
...
template <int N>
__device__ void
fft_kernel_r64_b16_half_branch(half2 *reg, const half2 *__restrict__ W_ptr) {
    half2 reg_frag_zero[TC_M_DEVICE_CONST * TC_N_DEVICE_CONST /
                        WARP_SIZE_DEVICE_CONST / 2];
...
#pragma unroll
        for (int j = 0; j < N_DEVICE_CONST / RADIX_DEVICE_CONST; j++) {
            ...
            fill_reg_b_half_branch<N, false>(reg_frag_b, i * 2, stride, i_perm,
                                             j_perm, k, W_ptr);
            // printf("%d %d %d %d %d : %f %f\n", threadIdx.x, stride, i_perm,
...
__global__ void fft_kernel_radix64_batch16_half(half2 *d_data,
    ...
    for (int i = 0; i < ept / 2; i++) {
        if ((laneid % 4) < 2) {
            reg[i] = make_half2(
                s_data[laneid / 4 * N + i * 4 + (laneid % 2) * 2].x,
                s_data[laneid / 4 * N + i * 4 + (laneid % 2) * 2 + 1].x);
            reg[i + ept / 2] = make_half2(
                s_data[(laneid / 4 + 8) * N + i * 4 + (laneid % 2) * 2].x,
                s_data[(laneid / 4 + 8) * N + i * 4 + (laneid % 2) * 2 + 1].x);
        } else {
            reg[i] = make_half2(
                s_data[laneid / 4 * N + i * 4 + (laneid % 2) * 2].y,
                s_data[laneid / 4 * N + i * 4 + (laneid % 2) * 2 + 1].y);
            reg[i + ept / 2] = make_half2(
                s_data[(laneid / 4 + 8) * N + i * 4 + (laneid % 2) * 2].y,
                s_data[(laneid / 4 + 8) * N + i * 4 + (laneid % 2) * 2 + 1].y);
        }
    }
...
#pragma unroll 1
    ...
    for (int i = 0; i < ept / 2; i++) {
        if ((laneid % 4) < 2) {
            ...
            s_data[(laneid / 4 + 8) * N + i + (laneid % 2) * 32].x =
                reg[i + ept / 2].x;
            s_data[(laneid / 4 + 8) * N + i + 16 + (laneid % 2) * 32].x =
                reg[i + ept / 2].y;
        } else {
            s_data[laneid / 4 * N + i + (laneid % 2) * 32].y = reg[i].x;
            s_data[laneid / 4 * N + i + 16 + (laneid % 2) * 32].y = reg[i].y;
            s_data[(laneid / 4 + 8) * N + i + (laneid % 2) * 32].y =
                reg[i + ept / 2].x;
            s_data[(laneid / 4 + 8) * N + i + 16 + (laneid % 2) * 32].y =
                reg[i + ept / 2].y;
        }
    }
...
__global__ void fft_kernel_radix64_batch16_half_branch(
    half2 *d_data, const half2 *__restrict__ W_64, unsigned int repeat) {
    // Tensor core shape
    constexpr int m = 16;
    constexpr int n = 8;
    constexpr int k = 8;
...
#pragma unroll 1
    ...
    for (int i = 0; i < ept; i++) {
        if ((laneid % 4) < 2) {
            s_data[laneid / 4 * N + i / 2 + (i & 1) * 16 + (laneid % 2) * 32]
                .x = reg[i];
            s_data[(laneid / 4 + 8) * N + i / 2 + (i & 1) * 16 +
                   (laneid % 2) * 32]
                .x = reg[i + ept];
        } else {
            s_data[laneid / 4 * N + i / 2 + (i & 1) * 16 + (laneid % 2) * 32]
