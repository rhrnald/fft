    ...
    if (local_row < tile_size) {
      for (int i = 0; i < (tile_size / 2 + 1 - 1) / fft_r2c::stride + 1; i++) {
        ...
        if (col < tile_size / 2 + 1)
          tile[col + (tile_size / 2 + 1) * local_row] = thread_data[i];
      }
    }
  }
  ...
  for (int j = 0; j < (tile_size / 2 + 1 - 1) / fpb + 1; j++) { // 17->16
    ...
    if (col < tile_size / 2 + 1) {
      for (int i = 0; i < fft_c2c_forward::elements_per_thread; i++) {
        thread_data[i] = tile[(local_thread_id + fft_c2c_forward::stride * i) *
                                  (tile_size / 2 + 1) +
                              col];
      }
    }
  ...
  for (int j = 0; j < (valid_tile_size - 1) / fpb + 1; j++) {
    ...
    if (local_row < valid_tile_size && global_row < output_size) {
      for (int i = 0; i < (tile_size / 2 + 1 - 1) / fft_r2c::stride + 1; i++) {
        int col = local_thread_id + fft_r2c::stride * i;
        if (col < tile_size / 2 + 1) {
          thread_data[i] = tile[col + (tile_size / 2 + 1) * local_row];
...
void FFTconv(real_type *d_input, cufftdx::complex<real_type> *d_filter,
  ...
  constexpr size_t total_shared_mem =
      fft_c2c_forward::shared_memory_size +
      sizeof(complex_type) * tile_size * (tile_size / 2 + 1);
  cudaFuncSetAttribute(conv_kernel<real_type, fft_r2c, fft_c2r, fft_c2c_forward,
  ...
  conv_kernel<real_type, fft_r2c, fft_c2r, fft_c2c_forward, fft_c2c_inverse,
              fft_single><<<tile_grid, fft_r2c::block_dim, total_shared_mem>>>(
      d_input, d_filter, d_output, N, f);
  timer.Stop();
