...
void convolution_cudnn(float *h_input, float *h_filter, float *h_output, int N,
    ...
    checkCudnn(cudnnSetConvolution2dDescriptor(conv_desc, 0, 0, 1, 1, 1, 1,
    ...
    checkCudnn(cudnnConvolutionForward(
        handle, &alpha, in_desc, d_input, filter_desc, d_filter, conv_desc,
        algo, workspace, workspace_bytes, &beta, out_desc, d_output));
    timer.Stop();
