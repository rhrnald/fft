...
#define CHECK_CUFFT(call)                                                      \
    do {                                                                       \
        ...
        if (err != CUFFT_SUCCESS) {                                            \
            std::cerr << "[cuFFT ERROR] " << #call << " failed with code "     \
                      << err << std::endl;                                     \
...
//             checkCuda(cudaMemcpy(host_result, d_result_tile,
//                                  sizeof(float) * T * T,
//                                  cudaMemcpyDeviceToHost));

//             for (int i = 0; i < stride && (ty * stride + i) < out_size; ++i) {
//                 for (int j = 0; j < stride && (tx * stride + j) < out_size;
//                      ++j) {
//                     h_output[(ty * stride + i) * out_size + (tx * stride + j)] =
//                         host_result[(i + f - 1) * T + (j + f - 1)] /
//                         (float)(T * T);
//                 }
//             }
//             free(host_result);
