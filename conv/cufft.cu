...
void convolution_cufft(float *h_input, float *h_filter, float *h_output, int N,
    ...
    checkCufft(cufftSetStream(planR2C, stream), "set stream r2c");

    GpuTimer timer(stream);
    timer.Start();

    checkCufft(cufftExecR2C(planR2C, d_input, d_input_fft), "exec r2c input");
    checkCufft(cufftExecR2C(planR2C, d_filter, d_filter_fft), "exec r2c filter");

    int n = N * (N / 2 + 1);
    pointwise_multiply<<<(n + 255) / 256, 256, 0, stream>>>(d_input_fft,
                                                          d_filter_fft, n);
    ...
    checkCuda(cudaMemcpy(tmp, d_result, sizeof(float) * N * N,
...
__global__ void store_tile(const float *__restrict__ result_tile,
    ...
    if (x < T - f + 1 && y < T - f + 1 && out_x < out_size &&
        out_y < out_size) {
...
//     GpuTimer timer;
//     timer.Start();
//     checkCufft(cufftExecR2C(planR2C, d_filter, d_filter_fft),
//                "exec r2c filter");
...
//             copy_tile<<<grid, block>>>(d_input, d_tile, N, T, f, tx, ty,
//                                        stride);
//             checkCuda(cudaDeviceSynchronize());

//             checkCufft(cufftExecR2C(planR2C, d_tile, d_input_fft),
//                        "exec r2c input");

//             int n = T * (T / 2 + 1);
//             pointwise_multiply<<<(n + 255) / 256, 256>>>(d_input_fft,
//                                                          d_filter_fft, n);
//             checkCuda(cudaDeviceSynchronize());

//             checkCufft(cufftExecC2R(planC2R, d_input_fft, d_result_tile),
//                        "exec c2r");

//             store_tile<<<grid, block>>>(d_result_tile, d_output, out_size, T, f,
//                                         tx, ty, stride);
//             checkCuda(cudaDeviceSynchronize());
//         }
//     }
...
//                          sizeof(float) * out_size * out_size,
//                          cudaMemcpyDeviceToHost));
